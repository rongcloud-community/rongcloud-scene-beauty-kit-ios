...
@implementation RCSBeautySlider
...
- (void)configureUI {
    [self setThumbImage:RCSBeautyImageNamed(@"slider_dot") forState:UIControlStateNormal];
...
- (void)setValue:(float)value animated:(BOOL)animated {
    ...
    if (_bidirection) {
        ...
        CGFloat originX =
            currentValue > 0 ? CGRectGetWidth(self.bounds) / 2.0 : CGRectGetWidth(self.bounds) / 2.0 - width;
...
- (UIImageView *)tipBackgroundImageView {
    if (!_tipBackgroundImageView) {
        UIImage *bgImage = RCSBeautyImageNamed(@"slider_tip_background");
...
- (UIView *)middleLine {
    if (!_middleLine) {
        _middleLine = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) / 2.0 - 1,
                                                               CGRectGetHeight(self.bounds) / 2.0 - 4, 2, 8)];
